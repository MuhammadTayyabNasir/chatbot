...
class ChatState {
  ...
  final String searchQuery; // --- NEW: For search functionality
  ...
  // --- NEW: Getter for filtered messages ---
  Map<DateTime, List<Message>> get filteredMessagesByDate {
    if (searchQuery.isEmpty) {
      return messagesByDate;
    }
    final filteredGroups = <DateTime, List<Message>>{};
    for (var entry in messagesByDate.entries) {
      final date = entry.key;
      final messages = entry.value;
      final filteredMessages = messages.where((msg) {
        final textMatch = msg.text.toLowerCase().contains(searchQuery.toLowerCase());
        final attachmentMatch = msg.attachment?.fileName.toLowerCase().contains(searchQuery.toLowerCase()) ?? false;
        return textMatch || attachmentMatch;
      }).toList();

      if (filteredMessages.isNotEmpty) {
        filteredGroups[date] = filteredMessages;
      }
    }
